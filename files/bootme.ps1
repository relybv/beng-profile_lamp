...
# This script installs puppet 4.x and deploy the manifest using puppet apply -e "include profile_lamp"
# Usage:
# <powershell>
# Set-ExecutionPolicy Unrestricted -Force
# icm $executioncontext.InvokeCommand.NewScriptBlock((New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/relybv/beng-profile_lamp/master/files/bootme.ps1')) -ArgumentList ("profile_lamp")
#</powershell>


  param(
    [string]$role = "profile_lamp"
  )

  $puppet_source = "https://github.com/relybv/beng-profile_lamp.git"
  ...
  if(!(Test-Path -Path "C:\ProgramData\PuppetLabs\code\modules\profile_lamp" )){
    $clone_args = @("clone",$puppet_source,"C:\ProgramData\PuppetLabs\code\modules\profile_lamp" )
  } else {
    $clone_args = @("pull")
    Set-Location -Path "C:\ProgramData\PuppetLabs\code\modules\profile_lamp"
  }
